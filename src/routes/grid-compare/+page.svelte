<script lang="ts">
	...
	import { IconGrid } from '@openmeteo/weather-map-layer';
	import * as maplibregl from 'maplibre-gl';
	import 'maplibre-gl/dist/maplibre-gl.css';

	import { getStyle } from '$lib/map-controls';

	// Debug view for the native ICON R3B07 grid (issue #278): draws the
	// analytical IconGrid triangles next to the actual DWD grid (vertices +
	// topology extracted from icon_grid_0026_R03B07_G.nc, served by
	// icon-native-test/serve.mjs) plus the centre displacement, so the
	// spring-dynamics warp between the two can be inspected visually.
	...
	let showAnalytical = $state(true);
	let showActual = $state(true);
	let showDisplacement = $state(true);

	const grid = new IconGrid({ type: 'icon', nx: N, ny: 1, iconRoot: 3, iconBisections: 7 });
	...
	const visibleCells = (bounds: maplibregl.LngLatBounds, centerCell: number): number[] => {
		...
		while (queue.length && visited.size < MAX_CELLS * 3) {
			...
			for (let e = 0; e < 3; e++) {
				const nb = neighborAcross(center, verts[e], verts[(e + 1) % 3]);
				if (!visited.has(nb)) {
					visited.add(nb);
					queue.push(nb);
				}
	...
	const update = () => {
		...
		if ((area / ((4 * Math.PI) / N)) * 1.2 > MAX_CELLS) {
			tooManyCells = true;
			for (const s of ['analytical-mesh', 'actual-mesh', 'displacement', 'true-centers']) {
		...
		const cells = visibleCells(bounds, grid.findCell(c.lat, c.lng));
		const lonRef = c.lng;

		const analytical: GeoJSON.Feature[] = [];
		...
		set('analytical-mesh', analytical);
	...
	const setVisibility = () => {
		if (!map) return;
		map.setLayoutProperty('analytical-mesh', 'visibility', showAnalytical ? 'visible' : 'none');
	...
	onMount(async () => {
		...
		map.on('load', () => {
			...
			map!.addLayer(lineLayer('analytical-mesh', '#2563eb', 1.2));
			map!.addLayer(lineLayer('actual-mesh', '#dc2626', 1.2));
			const displacementLayer = lineLayer('displacement', '#111', 2);
			(displacementLayer.paint as Record<string, unknown>)['line-color'] = [
				'interpolate',
				['linear'],
				['get', 'km'],
				0,
			...
			map!.addLayer(displacementLayer);
...
<svelte:head>
	<title>ICON grid compare — analytical vs actual</title>
</svelte:head>

<div class="page">
	<div class="map" bind:this={mapContainer}></div>
	<div class="panel">
		<h1>ICON R3B07: analytical vs actual</h1>
		<label
			><input type="checkbox" bind:checked={showAnalytical} /> <span class="swatch blue"></span> analytical
			grid (IconGrid)</label
		>
		<label
		...
		{:else}
			<p>
				{stats.cells} cells — deviation mean {stats.meanKm.toFixed(1)} km, max
				{stats.maxKm.toFixed(1)} km
			</p>
		{/if}
		<p class="hint">
			displacement colour: <span style="color:#22c55e">0</span> ·
			<span style="color:#eab308">30</span> · <span style="color:#dc2626">65 km</span>. Full
