...
export const prefetchData = async (
	...
	try {
		...
		// atomic per call, so the targets simply run sequentially to keep the
		// request volume per timestep bounded.
		const prefetchSingle = async (
			...
			for (const target of targets) {
				...
				if (target.warmOnly) {
					// Caches the file header/trailer only; no variable data is requested.
					// Best-effort: a sub-layer may have no file for this timestep.
					await reader.warmFile(url).catch(() => {});
					continue;
				}

				attempted++;
				try {
					await reader.prefetchVariable(url, variable, target.ranges, signal);
					succeeded++;
				} catch {
		...
		const worker = async () => {
			// Each worker's reader shares the protocol's block cache, so the warmed
			// blocks are reused by the renderer.
			...
			try {
				...
				// Reads are atomic and dispose their own file readers; only the
				// per-URL backend cache needs releasing.
				reader.clearBackends();
			}
			return localSuccess;
		};
