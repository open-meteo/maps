...
<!-- Fill of the GPU texture cache (top) and the data block cache (bottom) -->
<div
	class="flex flex-col justify-center gap-0.75 w-9 h-4.5 pr-0.5"
	title={`VRAM ${mb(vram.bytes)} / ${mb(vram.budgetBytes)} MB · RAM ${mb(ram.persistentBytes)} / ${mb(ram.maxBytes)} MB`}
>
