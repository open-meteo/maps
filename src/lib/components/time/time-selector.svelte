	...
	>
		...
		>
			...
			{#if $metaJson}
				{#if hoverX && desktop.current}
					<div
						...
						class="absolute shadow-md -top-8 p-0.5 w-16.5 text-center rounded bg-glass/65 backdrop-blur-sm {hoveredHour &&
						currentTimeStep &&
					...
					>
						<div class="relative {!timeValid(hoveredHour) ? 'text-foreground/50' : ''}">
							...
							{/if}
							<div
								class="-z-10 absolute -bottom-2 w-3 h-3 bg-glass/65 backdrop-blur-sm rotate-45 -translate-x-1/2 left-1/2"
					...
					>
						<div class="relative duration-500 {!disabled ? 'text-foreground' : ''}">
							...
							{#if disabled && desktop.current}
								<div
									transition:fade={{ duration: 200 }}
									class="-z-10 absolute -bottom-2 w-3 h-3 bg-glass/65 rotate-45 backdrop-blur-sm -translate-x-1/2 left-1/2"
									style="clip-path: polygon(0% 100%, 100% 100%, 100% 0%);"
								></div>
							{/if}
						</div>
			...
			{:else if desktop.current}
				...
				<div
					transition:fade={{ duration: 200 }}
					class="absolute flex items-center justify-center bg-glass/65 h-4.5 backdrop-blur-sm -top-6 rounded-none! p-0.5 w-16.5 text-center"
