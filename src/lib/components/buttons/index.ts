...
export class MoreButtons {
	private open = false;

	onAdd(map: maplibregl.Map) {
		const container = document.createElement('div');
		container.className = 'maplibregl-ctrl maplibregl-ctrl-group more-buttons-container';
		container.style.display = 'flex';
		container.style.flexDirection = 'column';

		const moreSVG = `<button style="display:flex;justify-content:center;align-items:center;">
			<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-ellipsis"><circle cx="12" cy="12" r="1"/><circle cx="19" cy="12" r="1"/><circle cx="5" cy="12" r="1"/></svg>
		</button>`;

		const mainButton = document.createElement('div');
		mainButton.innerHTML = moreSVG;
		mainButton.title = 'More';
		container.appendChild(mainButton);

		const subButtons = document.createElement('div');
		subButtons.style.display = 'none';
		subButtons.style.flexDirection = 'column';
		container.appendChild(subButtons);

		const snapshot = new SnapshotButton().onAdd(map);
		const darkmode = new DarkModeButton().onAdd();
		const help = new HelpButton().onAdd();

		[snapshot, darkmode, help].forEach((btn) => {
			btn.classList.remove('maplibregl-ctrl', 'maplibregl-ctrl-group');
			subButtons.appendChild(btn);
		});

		mainButton.addEventListener('click', () => {
			this.open = !this.open;
			subButtons.style.display = this.open ? 'flex' : 'none';
		});
